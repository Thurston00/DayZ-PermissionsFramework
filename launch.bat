...
IF %multiplayer%==1 (
    ...
    chdir /d "%serverDir%"
    start DZSALModServer.exe -scrAllowFileWrite -config=serverDZ.cfg -port=2302 "-profiles=%profiles%" -dologs -adminlog -freezecheck -password=abc123 -scriptDebug=true -cpuCount=4 "-mod=@RPCFramework;%modName%"
